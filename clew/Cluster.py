...
class Cluster:
    ...
    def analyce_phasing(self, phasing_threshold=100000, include_multi=True):
        self.count_instance('Phased regions', 0)

        if not self.is_filtered:
            return

        if self.counts['Chromosomes'] > 1 and not include_multi:
            return
    ...
    def interactions_distances(self, interaction_threshold=interaction_threshold, only_single_chorm=True,
        ...
        if only_single_chorm and len(self.occupied_chromosomes) > 1:
            return
