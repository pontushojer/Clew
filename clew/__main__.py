...
def get_argument_parser():
    ...
    parser.add_argument('-o', '--output', help='Output file prefix', default=None)
    parser.add_argument('-c', '--cluster_data', help='Output file with cluster data', default=False, action='store_true')
    parser.add_argument('-b', '--bed', help='Output bed file with cluster fragments.', default=False,
                        action='store_true')
    parser.add_argument('--debug', help='Debug', default=False, action='store_true')
    parser.add_argument('-i', '--interactions', help='Include interaction analysis', default=False, action='store_true')
    parser.add_argument('-p', '--phasing', help='Include phasing analysis', default=False, action='store_true')
    parser.add_argument('--stats', help='Include extra cluster stats', default=False, action='store_true')

    parser.add_argument('--dist-only', help='Include interaction analysis but only output distance information',
                        default=False, action='store_true')
    parser.add_argument('--insert', help='Include insert size output file.', default=False, action='store_true')
    parser.add_argument('-s', '--singles', help='Remove single alingments in analysis', default=False,
                        action='store_true')
    parser.add_argument('--separator', help='Output file separator', default=',', type=str)
    parser.add_argument('-q', '--quiet', help='No verbosity', action='store_true', default=False)
...
def main():
    ...
    if args.cluster_data or args.bed or args.interactions:
        if not args.output:
    ...
    for cluster_id, cluster_object in cluster_instances.items():
        ...
        if args.phasing:
            cluster_object.analyce_phasing(phasing_threshold=10000)

        if args.interactions:
            cluster_object.interactions_distances(only_single_chorm=False, interchromsomal=False)
        ...
        if args.bed and not args.debug:
            # Filter for only cluster carrying multiple fragments
        ...
        if args.dist_only:
            ...
            with open(prefix + ".interactions.csv", "a") as file2:
                ...
                for interaction in Cluster.total_interactions:
                    ...
                    file2.write(separator.join(line_to_write) + '\n')
